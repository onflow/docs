...
<Callout type="info">
To create accounts and generate keys, make sure to install [Flow CLI](../../tooling/flow-cli/install). There are a lot of convenient functions the flow CLI provides
</Callout>
...
Make sure to install the Flow CLI. [Flow CLI](../../tooling/flow-cli/accounts/create-accounts) has a interactive mode for generating keys, using the testnet faucet to create and fund the new account.

### Creating and deploying a Project

Flow CLI can be used to create a Cadence project and stay organized, [Flow CLI: Create a project](../../tooling/flow-cli). This will make deployment much easiler and help with the iterative development process.

After you have a project created and want to deploy your Cadence; contracts, transactions and scripts.[Flow CLI: deploy](../../tooling/flow-cli/deployment/deploy-project-contracts.md).
...
Flow Testnet comes with some useful contracts already deployed, called **core contracts.** More information and import addresses for the [core contracts](../../concepts/core-contracts/index.md).

      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 items-start">
        {sections.map((section, idx) => (
          <div key={idx} className="flex flex-col h-full items-start">
            {section.title && (
              <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">{section.title}</h3>
            )}
              ...
              {section.cards.map((card, cardIdx) => (
                <Tooltip key={cardIdx} description={card.description}>
                  <a
                    ...
                    className="flex items-center gap-3 text-base text-gray-900 dark:text-gray-100 hover:text-primary-green-500 dark:hover:text-primary-green-300 transition-colors"
                  >
                    <span className="flex items-center justify-center w-6 h-6 mr-2 shrink-0">
                      {card.icon && <Icon name={card.icon} className="w-6 h-6 flex-none" />}
                    </span>

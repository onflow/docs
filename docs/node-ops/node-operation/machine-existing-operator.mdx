...
The [Flow Epoch Preparation Protocol](../../references/run-and-secure/staking/04-epoch-preparation.mdx) requires that 
`collection` and `consensus` nodes use an automated [machine account](../../references/run-and-secure/staking/09-qc-dkg.mdx#machine-accounts) 

...
The [Flow Epoch Preparation Protocol](../../architecture/staking/04-epoch-preparation.mdx) requires that 
`collection` and `consensus` nodes use an automated [machine account](../../architecture/staking/09-qc-dkg.mdx#machine-accounts) 
...
head over to [Flow Port](../../building-on-flow/run-and-secure/nodes/flow-port/staking-guide.md#existing-node-operators) to submit a transaction to create a Machine Account. 

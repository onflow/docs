  ...
  return (
    <FlowProvider
      config={{
        ...
        discoveryWallet: 'https://fcl-discovery.onflow.org/fcl/testnet',
      }}
      flowJson={flowJSON}
    >
      <App />
...
All UI components in `@onflow/react-sdk` are styled using [Tailwind CSS](https://tailwindcss.com/) utility classes with flow prefix. The kit supports both light and dark themes out of the box, using Tailwind's `dark:` variant for dark mode styling.
...
<FlowProvider
  ...
  theme={{
    colors: {
      primary: {
        background: "flow-bg-blue-600 dark:flow-bg-blue-400",
        text: "flow-text-white dark:flow-text-blue-900",
        hover: "hover:flow-bg-blue-700 dark:hover:flow-bg-blue-300",
      },
...
Dark mode is **fully controlled by the parent app** using the `colorMode` prop on `FlowProvider`.

- `colorMode={'system'}` (default): Matches the system color scheme, if the system is in light mode it will be light, otherwise dark.
- `colorMode={'light'}`: Forces all the react-sdk components to use light mode styles.
- `colorMode={'dark'}`: Forces all the react-sdk components to use dark mode styles.

- You can dynamically change the `colorMode` prop to switch themes at runtime.
...
function App() {
  ...
  return (
    <FlowProvider config={...} colorMode={isDark ? 'dark' : 'light'}>
      <MyFlowComponents />
    </FlowProvider>
  )
}
...
You can use the `useDarkMode` hook to check the current value set inside your components:
...
- All react-sdk components will automatically apply the correct Tailwind `dark:` classes based on the `colorMode` prop.
- For best results, ensure your app's global theme and the kit's `colorMode` prop are always in sync.
...
<FlowProviderDemo>
  <TransactionLink txId="0x1234567890abcdef" variant="primary" />
</FlowProviderDemo>
...
<Callout type="info">
  This feature is currently only supported on Testnet & Mainnet networks.
  Emulator support will be added in a future release.
</Callout>
...
function UseCrossVmTokenBalanceExample() {
  ...
  return (
    <div>
      ...
      <p>
        Combined Balance: {data.combined.formatted} (Value:{' '}
        {data.combined.value})
      </p>
      <button onClick={refetch}>Refetch</button>
    </div>
...
```typescript
interface UseCrossVmSpendNftResult
  extends Omit<
...
```typescript
interface UseCrossVmSpendTokenResult
  extends Omit<

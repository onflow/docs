...
| `domainTag` | string                                                                                                   | ✅       | Domain separation tag provided as a utf-8 encoded string (default: no domain separation tag). See more about [domain tags here](../../../building-on-flow/cadence/language/crypto.mdx#hashing-with-a-domain-tag).                             |
...
| `weight`             | No       | number                                            | Weight of the key - see [Flow Core Concepts](../../../building-on-flow/concepts/start-here/accounts-and-keys.md#keys) for more information |

### PublicKey

Public keys are stored as `Buffer` objects which have been RLP encoded according to the [Flow spec](../../../building-on-flow/concepts/start-here/accounts-and-keys.md).

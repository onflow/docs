...
📖 **gRPC Access API URLs** can be found [here](../../../building-on-flow/concepts/nodes/access-api.mdx#flow-access-node-endpoints). `sdk.transport` must be specified if you wish to use the gRPC API.  The public Flow gRPC access nodes are accessible at:
...
Please read more about [events in the documentation](../../../building-on-flow/cadence/language/core-events.md). The exception to this standard are 
...
Scripts allow you to write arbitrary non-mutating Cadence code on the Flow blockchain and return data. You can learn more about [Cadence here](../../../building-on-flow/cadence/language/index.md) and [scripts here](./scripts.mdx), but we are now only interested in executing the script code and getting back the data.
...
📖 **[Proposal key](../../../building-on-flow/concepts/start-here/transaction-signing.md#proposal-key)** must be provided to act as a sequence number and prevent replay and other potential attacks.

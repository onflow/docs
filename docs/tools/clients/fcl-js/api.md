...
**Note:** The default values for `proposer`, `payer`, and `authorizations` are already `fcl.authz` so there is no need to include these parameters, it is shown only for example purposes. See more on [signing roles](../../../building-on-flow/concepts/start-here/accounts-and-keys.md#signing-a-transaction).
...
These methods allows dapps to interact directly with the Flow blockchain via a set of functions that currently use the [Access Node API](../../../building-on-flow/concepts//nodes/access-api.mdx).
...
| `limit`   | number                                | Compute (Gas) limit for query. Read the [documentation about computation cost](../../../building-on-flow/concepts/start-here/variable-transaction-fees.md) for information about how computation cost is calculated on Flow. |
...
| `authorizers`        | [\[Address\]](#address)               | A list of the accounts that are authorizing this transaction to mutate to their on-chain account state.  [See more here](../../../building-on-flow/concepts/start-here/transaction-signing.md#signer-roles). |
| `envelopeSignatures` | [\[SignableObject\]](#signableobject) | A list of signatures generated by the payer role. [See more here](../../../building-on-flow/concepts/start-here/transaction-signing.md#anatomy-of-a-transaction).                                            |
| `gasLimit`           | number                                | The maximum number of computational units that can be used to execute this transaction.  [See more here](../../../building-on-flow/concepts/start-here/variable-transaction-fees.md).                                  |
| `payer`              | [Address](#address)                   | The account that pays the fee for this transaction.  [See more here](../../../building-on-flow/concepts/start-here/transaction-signing.md#signer-roles).                                                     |
| `payloadSignatures`  | [\[SignableObject\]](#signableobject) | A list of signatures generated by the proposer and authorizer roles. [See more here](../../../building-on-flow/concepts/start-here/transaction-signing.md#anatomy-of-a-transaction).                         |
...
For more on what each transaction role means, see [singing roles](../../../building-on-flow/concepts/start-here/transaction-signing.md#signer-roles).
...
You can find more information about sequence numbers [here](../../../building-on-flow/concepts/start-here/transaction-signing.md#sequence-numbers)

...
description: Type definitions for the Flow Client Library (FCL).
---
...
| `message` | `{ cadence: string; refBlock: string; computeLimit: number; proposer: string; payer: string; authorizations: string[]; params: Record<string, any>[]; arguments: string[]; }` |  |
| `proposer` | `string` |  |
| `authorizations` | `string[]` |  |
| `payer` | `string[]` |  |
| `events` | `{ eventType: string; start: string \| number; end: string \| number; blockIds: string[]; }` |  |
| `transaction` | `{ id: string; }` |  |
| `block` | `{ id: string; height: string \| number; isSealed: boolean; }` |  |
| `account` | `{ addr: string; }` |  |
| `collection` | `{ id: string; }` |  |
| `subscribeEvents` | `{ eventTypes: string[]; addresses: string[]; contracts: string[]; startBlockId: string; startHeight: number; heartbeatInterval: number; }` |  |
...
| `signingFunction` | `any` |  |
| `resolve` | `any` |  |
| `role` | `{ proposer: boolean; authorizer: boolean; payer: boolean; param?: boolean; }` |  |
| `authorization` | `any` |  |

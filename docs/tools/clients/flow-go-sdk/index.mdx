...
Please read more about [events in the documentation](../../../building-on-flow/core-contracts/03-flow-token.md). The exception to this standard are
...
📖 **Script** field is the portion of the transaction that describes the state mutation logic. On Flow, transaction logic is written in [Cadence](../../../overview/about/cadence.md). Here is an example transaction script:
...
📖 **[Proposal key](../../../concepts/transactions.md#proposal-key)** must be provided to act as a sequence number and prevent reply and other potential attacks.
...
📖 **[Payer](../../../concepts/transactions.md#signer-roles)** is the account that pays the fees for the transaction. A transaction must specify exactly one payer. The payer is only responsible for paying the network and gas fees; the transaction is not authorized to access resources or code stored in the payer account.

📖 **[Authorizers](../../../concepts/transactions.md#signer-roles)** are accounts that authorize a transaction to read and mutate their resources. A transaction can specify zero or more authorizers, depending on how many accounts the transaction needs to access.
...
Cadence uses metering to measure the number of operations per transaction. You can read more about it in the [Cadence documentation](../../../overview/about/cadence.md).
...
Before trying the examples below, we recommend that you read through the [transaction signature documentation](../../../concepts/transactions.md.

After you have successfully [built a transaction](#build-the-transaction) the next step in the process is to sign it. Flow transactions have envelope and payload signatures, and you should learn about each in the [signature documentation](../../../concepts/transactions.md).
...
### [Single party, single signature](../../../concepts/transactions.md#single-party-single-signature)
...
### [Single party, multiple signatures](../../../concepts/transactions.md#single-party-multiple-signatures)
...
### [Multiple parties](../../../concepts/transactions.md#multiple-parties)
...
### [Multiple parties, two authorizers](../../../concepts/transactions.md#multiple-parties)
...
### [Multiple parties, multiple signatures](../../../concepts/transactions.md#multiple-parties)
...
The Flow VM uses a deterministic address generation algorithm to assign account addresses on chain. You can find more details about address generation in the [accounts & keys documentation](../../../concepts/accounts.md).
...
A Flow account can contain zero (not possible to control) or more public keys, referred to as account keys. Read more about [accounts in the documentation](../../../concepts/accounts.md).
...
[<img src="https://raw.githubusercontent.com/onflow/sdks/main/templates/documentation/ref.svg" width="130" />](../../../concepts/accounts.md#signature-and-hash-algorithms)
...
The example above uses an ECDSA key pair on the P-256 (secp256r1) elliptic curve. Flow also supports the secp256k1 curve used by Bitcoin and Ethereum. Read more about [supported algorithms here](../../../concepts/accounts.md#signature-and-hash-algorithms).

...
For new node operators, Machine Accounts are created during the [staking process](../../building-on-flow/run-and-secure/nodes/flow-port/staking-guide.md) in Flow Port.
For node operators who initially staked prior to the introduction of Machine Accounts, Machine Accounts can be
created for your staked nodes by following [this guide](../../building-on-flow/run-and-secure/nodes/node-operation/machine-existing-operator.mdx).

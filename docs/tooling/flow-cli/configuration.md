...
- A `deployments` empty object where all [deployment targets](./project-contracts.md#define-contract-deployment-targets) can be defined. 
...
We can specify aliases for each network we have defined. When deploying to testnet it is always a good idea to specify aliases for all the [common contracts](../../cadence/core-contracts/index.md) that have already been deployed to the testnet. 

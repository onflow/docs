...
You can query events through the Flow Access API using the available [Flow SDKs](/tools#sdks). Event data is returned in [JSON-Cadence format](../building-on-flow/cadence/json-cadence-spec).
...
You can execute scripts through the Flow Access API using the available [Flow SDKs](/tools#sdks). Like events, script results are returned in [JSON-Cadence format](../building-on-flow/cadence/json-cadence-spec).

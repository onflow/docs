...
for [the staking collection](../../building-on-flow/run-and-secure/staking/14-staking-collection.mdx)

# Events

The `FlowIDTableStaking` contract emits an event whenever an important action occurs.
See the [staking events Documentation](../../building-on-flow/run-and-secure/staking/07-staking-scripts-events.md) for more information about each event.

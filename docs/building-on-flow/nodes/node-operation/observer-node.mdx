...
└── data (directory used by the light client to store block data)
```
...
The `upstream-node-addresses` is the list of access node hostnames to which this light client can delegate to. The list can be different from the bootstrap node list.
...
The light client, like the other type of Flow nodes, also produces Prometheus metrics that can be used to monitor node health. More on that [here](./node-setup.mdx#monitoring-and-metrics)
...
```
`public-network-address` is the address the light clients will connect to.

...
When a field/index is read through a reference, it wil return:
...
// Whereas, if the array is of primitive type, it will return the concrete value.
var intArrayRef: &[Int] = ...
var element: Int = intArrayRef[2]
```

```cadence
// Index-access to a dictionary reference would return a refe rence to the value.
var resourceDictionaryRef: &{String: AnyResource} = ...
var valueRef: &AnyResource? = resourceDictionaryRef["two"]

// Whereas, if the dictionary values are of primitive type, it will return the concrete value.
...
Think of assignment as a built-in function with `Mutate` and `Insert` entitlements. e.g:

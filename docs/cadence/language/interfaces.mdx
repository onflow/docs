...
An interface can inherit (conform to) other interfaces of the same kind.
...
In the above example, `Vault` inherits `Receiver`. Anyone implementing the `Vault` interface would also have to
...
i.e: Replacing the `pre` conditions in the above example with `post` conditions with the exact same content would

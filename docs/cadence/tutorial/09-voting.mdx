...
Before proceeding with this tutorial, we highly recommend following the instructions in [Getting Started](./01-first-steps.mdx)
and [Hello, World!](./02-hello-world.mdx) to learn how to use the Playground tools and to learn the fundamentals of Cadence.
...
1. In the bottom right deployment modal, press the arrow to expand and make sure account `0x01` is selected as the signer.
1. Click the Deploy button to deploy it to account `0x01`
...
1. Submit the transaction with account `0x01` selected as the only signer.
...
1. Select account `0x01` as a signer first, then also select account `0x02`.
1. Submit the transaction by clicking the `Send` button
...
1. Select account `0x02` as the only transaction signer.

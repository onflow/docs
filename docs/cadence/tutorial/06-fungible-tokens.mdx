...
**in order to pay for [storage fees and transaction fees](../../concepts/flow-token/concepts.md#fees).**
...
1. Make sure that signer `0x02` is selected as the deploying address
1. Click "Deploy"
...
1. This transaction creates a new public reference to your `Vault` and checks that it was created correctly.
...
1. Select account `0x03` as the only signer.
1. Click the `Send` button to set up account `0x03` so that it can receive tokens.

```cadence Setup-Account.cdc
// Setup Account

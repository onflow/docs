...
1. With `ExampleMarketplace.cdc` open, select account `0x03` from the deployment modal in the bottom right and deploy.
...
1. Select account `0x01` as the only signer and click the `Send` button to submit the transaction.
...
1. Select account `0x02` as the only signer and click the `Send` button.

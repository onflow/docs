...
Both the [block height and the block timestamp](./language/environment-information#block-information.md) are accessible from within Cadence code.
...
as [the block rate naturally fluctuates](../concepts/nodes/faq/operators.mdx#does-the-blockheight-go-up-1-every-second).

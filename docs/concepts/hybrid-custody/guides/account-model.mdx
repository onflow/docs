...
Consider this your grimoire, fellow builder, where we’ll continue from the perspective of a wallet or marketplace dapp
seeking to facilitate a unified account experience, abstracting away the partitioned access between accounts into a
single dashboard for user interactions on all their owned assets.
...
access between the dapp which created and presumably maintains custody of the account and the linked parent account.

How does this delegation occur? Typically when we think of shared account access in crypto, we think keys. However,
Cadence enables [accounts to link Capabilities on themselves](https://github.com/onflow/cadence/issues/2151) and issue
...
dapp creates an account it controls, then later delegates access on that account to the user once they’ve authenticate
with their wallet. All related constructs are used together in the [`HybridCustody`
contract](https://testnet.contractbrowser.com/A.96b15ff6dfde11fe.HybridCustody) to define the standard. The delegated
access on child accounts is stored and mediated by the parent account's `Manager` which maintains Capabilities on
`ProxyAccount` and `ChildAccount` resources, themselves defining restricted and unrestricted access respectively.

Therefore, the presence of a `Manager` in an account implies there are potentially associated accounts for which the
owning account has delegated access. This resource is intended to be configured with a pubic Capability enabling
...
We can see that the user’s `Manager.accounts` point to the address of its child account in its index, with corresponding
Capabilities giving the Manager access to those accounts. Likewise, the child account’s `ProxyAccount.parentAddress`
points to the user’s account as its parent address. This makes it easy to both identify whether an account is a parent,
child, or both, and its associated parent/child account(s). `ChildAccount` resources underly all account delegations, so
can have multiple parents whereas `ProxyAccount`s are 1:1. This provides more granular revocation as each parent account
has its own Capability path on which its access relies.
...
configured by the delegator. The `ProxyAccount` maintains these rules along with a `ChildAccount` resource within which
the AuthAccount Capability is stored. Anyone with access to the surface level `ProxyAccount` can then access the
underlying AuthAccount, but only according the the pre-defined rule set. These rules are fundamentally a list of Types
that can/cannot be retrieved from an account.
...
Vaults, for example, the app developer can codify rule sets on allowable Capability types in a `CapabilityFilter` along
with a `CapabilityFactory` defining retrieval patterns for those Capabilities. When delegation occurs, they would
provide the `CapabilityFilter` and `CapabilityFactory` Capabilities to a `ChildAccount` which wraps the given filter &
factory Capabilities in a `ProxyAccount` along with a Capability to itself before publishing the new `ProxyAccount`
Capability for the specified parent account.
...
Flow Provider, enabling withdrawal from the linked account having signed with the main account.
...
approach could get you reference to the linked account’s 

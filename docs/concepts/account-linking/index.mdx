...
The idea of account ownership actually translates to the ability to access the auth account.

## AuthAccount Capabilities

Understanding [Cadence capabilities](../../cadence/language/capability-based-access-control) is required to follow this section.
...
Creating an AuthAccount capability and publishing that capability to another account is what we define as account linking. An account owning the `AuthAccount` capability of another account is called a parent account, and the account sharing the capability with the other account becomes its child account.
...
## How is account linking useful?

Account linking is very useful in the case of onboarding non-existing Flow users to your Flow application. Because participating on Flow network requires having a Flow account, which a non-web3-native user will struggle to achieve a lot of applications create the account for the user.

The problem with this approach becomes when a user wants to use that account with another application, or move the items stored in that account out of the application. With account linking it’s possible for the application to create the account instead of the user, but still allow for the user to later link that account to any other accounts he will own.

Account linking allows having multiple Flow accounts and abstracts them as a single account.

...
You can learn more about this division of identity [here](./node-operation/node-bootstrap.mdx#generating-your-node-id).
...
So, who gets slashed? Severe slashing on Flow only takes place in the most nefarious of attacks. The protocol reserves slashing for maintaining the security of the protocol rather than its liveness. You can find more details on the conditions under which a node is slashed [here](./staking/stake-slashing.md).
...
3. **Run the node** (see [Node Operation Section](./node-operation/node-setup.mdx))
...
2.  the stake is added to the Identity for the newly created node (the Identity Table is part of Flow's [_Protocol State_](./node-operation/node-bootstrap.mdx#generating-your-node-id), which is maintained by the Consensus Nodes)
3.  **At genesis, **Nodes are _initialized with the stake already accredited to their [Network Identity](./node-operation/node-bootstrap.mdx#generating-your-node-id)_, which is external to the Node Operator's Account.

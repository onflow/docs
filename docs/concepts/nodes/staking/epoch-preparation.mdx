...
For each Epoch, there is a [Staking Auction](./technical-overview.mdx) in which new potential node operators may submit Staking Commitments.
...
Specifically, it is allowed to emit [Service Events](./epoch-scripts-events.mdx#monitor-epoch-service-events),
...
See the [Epoch Setup Event Documentation](./epoch-scripts-events.mdx#epochsetup)
...
See the [Epoch Commit Event Documentation](./epoch-scripts-events.mdx#epochcommit)
...
See the [epoch scripts and events document](./epoch-scripts-events.mdx#introduction) for detailed documentation about

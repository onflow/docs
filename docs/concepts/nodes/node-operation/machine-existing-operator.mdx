...
The [Flow Epoch Preparation Protocol](../../../building-on-flow/run-and-secure/staking/04-epoch-preparation.mdx) requires that 
`collection` and `consensus` nodes use an automated [machine account](../../../building-on-flow/run-and-secure/staking/09-qc-dkg.mdx#machine-accounts) 

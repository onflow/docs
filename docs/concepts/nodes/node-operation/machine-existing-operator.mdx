...
The [Flow Epoch Preparation Protocol](../staking/epoch-preparation.mdx) requires that 
`collection` and `consensus` nodes use an automated [machine account](../staking/qc-dkg.mdx#machine-accounts) 
...
head over to [Flow Port](../flow-port/staking-guide.mdx#existing-node-operators) to submit a transaction to create a Machine Account. 

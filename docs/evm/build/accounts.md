...
3. **Cadence Owned Accounts (COA)**: This is an account type unique to Flow EVM. These accounts are owned by Cadence resources and can be used to interact with the Flow EVM from within the Cadence environment.
...
### Differences between COAs and other account types
...
- **Native Account Abstraction**: COAs are controlled by Cadence resources, which are in turn owned by Flow accounts. Flow accounts provide native support for multi-signature authentication, key rotation, and account recovery. As a Cadence resource, COAs naturally inherit [these features](https://developers.flow.com/build/differences-vs-evm/account-abstraction).

- **Fine-Grained Access Control**: As Cadence resources, access to a COA can be governed by more sophisticated policies than are avaialable with basic EVM accounts. By utilizing Cadence features such as [capabilities and entitlements](https://cadence-lang.org/docs/1.0/language/access-control) developers can choose to restrict exactly who can interact with a COA and what actions they are permitted to perform.

...
Scheduled  are a new feature that is under development and is a part of [FLIP 330]. Currently, they only work in the emulator. The specific implementation may change as a part of the development process.
...
- Analyze the structure and flow of scheduled transactions
...
We could move forward with this, but it would be more fun to have each transaction schedule the follow transaction to share the next message. You can do this by moving most of the code found in the callback transaction to the handler. Start with configuring the `delay`, `future`, `priority`, and `executionEffort`. We'll hardcode these for simplicity:
...
And execute the transaction to initialize the new scheduled transaction:

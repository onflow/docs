...
<Callout type="info">
To create accounts and generate keys, make sure to install [Flow CLI](../../build/getting-started/local-development/flow-cli/install). Flow CLI provides convenient functions to simplifies interacting with the blockchain.
</Callout>
...
Make sure to install the Flow CLI. [Flow CLI](../../build/getting-started/local-development/flow-cli/accounts/create-accounts) has a interactive mode for generating keys, using the testnet faucet to create and fund the new account.

### Creating and deploying a Project

Flow CLI can be used to create a Cadence project and stay organized, [Flow CLI: Create a project](../../build/getting-started/local-development/flow-cli). This will make deployment much easiler and help with the iterative development process.

After you have a project created and want to deploy your Cadence; contracts, transactions and scripts.[Flow CLI: deploy](../../build/getting-started/local-development/flow-cli/deployment/deploy-project-contracts.md).

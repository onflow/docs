...
Flow network is made up of many node types. Here is more information on the different [Node Types](../../concepts/nodes/node-operation/node-roles#collection). For a quick reference here are the types:
 - **Observer**: No stake required, Easist node to spin up. This node is used to observe the network and get the latest blockchain data.
 - **Access**: Staked node, Provides access to the Flow network. This node is used to send transactions to the network.

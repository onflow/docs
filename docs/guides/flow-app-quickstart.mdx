...
Before running the app the emulator needs to be run. This is a local blockchain [More Information](../tooling/emulator). In a console run this commmand:

```sh
flow emulator
```

Now run the app using the following command in another terminal, aka console.

```sh
npm run dev
```

Now that your app is running. FYI, there is a configuration file name `config/fcl.ts` This file contains configuration information for FCL, such as what Access Node and wallet discovery endpoint and which network to use (e.g. testnet or a local emulator). 
...
You should now be able to log in or sign up a user and unauthenticate them. Upon logging in or signing up your users will see a popup where they can choose between wallet providers. Let's select the [Lilico wallet](https://lilico.app/) for this example to create an account. Upon completing authentication, you'll see the component change and the user's wallet address appear on the screen if you've completed this properly.

## Deploying project contracts

You will notice no contracts have not been deployed. So the UI has nothing to communicate with. Hello World scaffold does come with a Cadence contract. 
...
Deploy contracts, In a console in root of Hello world app, run the following command.

```sh
flow project deploy
```

Verify that `flow.json` updates with HelloWorld contract information
...
Now that all project contracts have been deployed, the UI can query them.

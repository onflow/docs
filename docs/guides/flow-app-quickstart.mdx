...
<Callout type="info">
 If you are looking for a scalfolds, check out [scaffolds availabe in the Flow CLI](../build/getting-started/local-development/flow-cli/super-commands.md#using-scaffolds).
</Callout>
...
This guide assumes a good understanding of React. The concepts are easy to understand and apply to other libraries and framework. A strong understanding of Cadence (Flow's smart contract language) is not required. More information on Cadence, [learning the Cadence language](../overview/about/cadence.md).
...
Make sure you have Flow CLI installed. [installation instructions](../build/getting-started/local-development/flow-cli/install.md).
...
We don't recommend keeping private keys in your `flow.json`, notice that Flow CLI already moved the emulator private key to a `emulator.key` file and point to it using the [key/location pattern](../build/getting-started/local-development/flow-cli/flow.json/security.md#private-account-configuration-file). This file should be added to your .gitignore file, so it won't be committed to your repository. 
...
<Callout type="info">
Emulator is a local blockchain [More Information](../tools/toolchains/emulator). Emulator has all the features as testnet and mainnet blockchains 
</Callout>
...
The `accessNode.api` key specifies the address of a Flow access node. There are publically available access nodes, but have to rate limit. Alternatively, applications might want to run an Observer node [Run a Node](../node-ops/running-node.md).
...
**Cadence**
- [Cadence Playground Tutorials](../build/getting-started/hello-world/tutorial/01-first-steps.md)
